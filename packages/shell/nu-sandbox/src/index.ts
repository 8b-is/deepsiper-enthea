/**
 * Sandbox-consuming nushell executor. It wraps the exact local nushell argv
 * (`nu --no-config-file -c <command>`) through `ctx.sandbox`, inherits local
 * process mechanics, and reports the selected mode, enforcement, and denial
 * facts. Positive runner-launch evidence means the command never ran:
 * foreground calls throw `SANDBOX_UNAVAILABLE`, while background processes
 * carry `runnerFailed`; other spawn rejections retain local-executor
 * semantics. The tool owns approval and passes a complete per-call policy.
 * @module @deepseek-ai/dsh-nu-sandbox
 */
...
import type { Config as LocalConfig } from '@deepseek-ai/dsh-nu-local'
 ...
 */
export class SandboxNuExecutor extends LocalNuExecutor {
  static override inject = ['subprocess', 'sandbox', 'sandboxPolicy']

  // No own Config: the sandbox default (mode + workspaceRoot) is owned by
  // ctx.sandboxPolicy, so this executor inherits LocalNuExecutor's Config
  ...
  override async run(spec: ShellExecSpec): Promise<ShellRunResult> {
    ...
    const confined = this.confine(nuPath, spec.command, { ...policy, mode })
  ...
  override start(spec: ShellExecSpec): ShellProcess {
    ...
    const nuPath = this.currentNuPath
    if (nuPath === undefined) {
      throw new Error('nu-sandbox: the nushell binary is not resolved yet (nix build) — run a foreground command first')
    }
    const confined = this.confine(nuPath, spec.command, { ...policy, mode })
  ...
  /**
   * Wrap one nushell command via the `ctx.sandbox` provider. The wrapped argv
   * targets the Nix-provisioned nushell, so the sandbox runner confines the
   * same deterministic store-path binary every host resolves. Provider errors
   * propagate unchanged; the returned argv is handed directly to the local
   * executor's subprocess path.
   * @param nuPath - the resolved nushell executable path.
   * @param command - nushell source for the confined inner `nu -c`.
   * @param policy - resolved confined execution policy.
   * @returns the provider's exact argv and settlement-classification facts.
   */
  private confine(nuPath: string, command: string, policy: SandboxPolicy): ConfinedArgv {
    return this.ctx.sandbox.confine([nuPath, '--no-config-file', '-c', command], policy)
  }
}

export default SandboxNuExecutor
