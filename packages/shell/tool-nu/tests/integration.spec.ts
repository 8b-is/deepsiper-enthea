/**
 * Integration tests: the REAL `@deepseek-ai/dsh-nu-local` executor plus the
 * `nu` tool, exercised through `ctx.tools.execute()` with a real nushell
 ...
 * self-skips when no usable `nu` resolves; the fake-executor suite
...
const hasNu = spawnSync('nu', ['--no-config-file', '-c', 'true'], { encoding: 'utf8' }).status === 0

/** Normalize nushell's platform line endings (CRLF on Windows, LF elsewhere). */
...
describe.skipIf(!hasNu)('nu tool over the real nu executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'dsh-tool-nu-'))
    await writeFile(join(dir, 'greeting.txt'), 'hello nu\n')
    ...
    await ctx.plugin(LocalNuExecutor, { timeoutMs: 20_000, graceMs: 200, nuBin: 'nu' })
  ...
  it('runs a command and returns stdout with no marker on a clean exit', async () => {
    const result = await call('nu', { command: 'echo hi', description: 'say hi' }, agent())
    expect(result.isError).toBe(false)
    if (result.isError) throw new Error('expected nu success')
  ...
  it('returns stderr in a marked section and a nonzero exit as a marker, not an error', async () => {
    const result = await call('nu', {
      command: "print -e 'boom'; exit 3",
  ...
  it('resolves relative paths in the session workspace', async () => {
    const result = await call('nu', {
      command: 'open greeting.txt | print',
      description: 'read greeting',
    }, agent())
    expect(result.isError).toBe(false)
    expect(lf(text(result))).toBe('hello nu\n')
  })
  ...
  it('a background run settles through the REAL job_output tool', async () => {
    const started = await call('nu', {
      command: "sleep 300ms; print 'bg-done'",
      description: 'background greeting',
      run_in_background: true,
    })
    expect(started.isError).toBe(false)
    if (started.isError) throw new Error('expected background nu success')
