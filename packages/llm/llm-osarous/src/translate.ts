/**
 * Translate osarous SSE payloads with one stateful harness block per content,
 * reasoning, or tool-call index. An empty initial reasoning delta does not open
 * a block. Finish reason and the latest usage are deferred until `[DONE]`,
 * covering both finish-attached and trailing usage-only shapes while ensuring
 * no chunk follows `finish`.
 * @module dsh-llm-osarous/translate
 */
...
/**
 * Map wire usage fields. The wire `prompt_tokens` INCLUDES cache hits; the
...
export async function* translate(payloads: AsyncIterable<string>): AsyncGenerator<StreamChunk> {
  ...
  for await (const payload of payloads) {
    ...
    try {
      ...
      throw new LlmError('Malformed response from osarous', 'MALFORMED_RESPONSE', { cause: payload })
    }
