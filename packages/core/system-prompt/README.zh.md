...
| 键 | 默认值 | 含义 |
|---|---|---|
| `includeHarnessIdentity` | `true` | 是否包含顺序为 −100 的固定开场白 `You are an AI agent powered by DeepSeek Harness, an extensible plugin-based agent runtime.`。仅当兼容性部署拥有完整系统提示词时设为 false。 |
...
| `appendSystemPrompt` | `''` | 由部署编写、追加为系统提示词最后一段的文本（`deployment:append`，顺序 `1000`）——位于 persona 与所有工具引导段之后。`{{variable}}` 引用是严格的。为空或省略 ⇒ 不产生追加段。这是注入式提示词的部署端形态：已保存的指导落到系统提示词末尾，而不是进入用户输入。 |
| `contextTokenBudget` | 无 | 渲染后的运行时上下文快照的 token 上限。当渲染出的上下文超过该上限时，保留 order 最高的若干贡献，并追加一条 `context:truncated` 说明，使模型知道快照是局部的。省略 ⇒ 不做预算。token 估算使用 `estimateTokens` 启发式（`字符数 / 4`），因此预算需留足余量。仅限正整数；非正数在加载时失败。 |
...
- `PromptSection`：`{ name, order, text, complete? }`。各段按 `order` 升序拼接。顺序区间：`-100` 是 harness 身份，`0` 是部署 persona，工具引导使用 `100–199`，`1000` 是部署追加尾部。协作式组装完成后，一个有效的 `complete` 段会抑制其他所有段。
- `PromptAssembly`：`{ sections: AssembledSection[], tools: ToolSchema[], variables: Record<string, string | undefined>, contextTokenBudget? }`。各段文本到达时已求值，但尚未插值；`variables` 保存所有已注册变量在当前上下文中求得的值。工具 schema 按设计属于组装结果：「模型获知自己能做什么」是一个连贯整体，尽管适配器把 schema 作为独立 wire 字段传输。`contextTokenBudget` 是部署对渲染后的运行时上下文快照设置的 token 上限，由配置写入，由 `renderContextSections` 消费。
- `renderContextSections(assembly)` / `renderContextSnapshot(assembly)`：运行时上下文快照的命名贡献形式 / 拼接后散文（`Current runtime context…` 块）。当设置 `contextTokenBudget` 时，保留 order 最高的、能容纳的若干贡献，并追加一条 `context:truncated` 段以说明省略数量，使模型与任何 invariant 伴随模块都能看到快照是局部的。
...
- 段提供方：工具包拥有自身的跨调用指导（`tool:bash`、`tool:read` 等）；此插件拥有 `harness:identity`、`deployment:persona` 与已配置的 `deployment:append` 尾部。
