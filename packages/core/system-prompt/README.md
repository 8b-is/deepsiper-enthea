...
| Key | Default | Meaning |
|---|---|---|
| `includeHarnessIdentity` | `true` | Include the fixed `You are an AI agent powered by DeepSeek Harness, an extensible plugin-based agent runtime.` order-−100 opener. Set false only when a compatibility deployment owns the complete system prompt. |
...
| `appendSystemPrompt` | `''` | Deployment-authored text appended as the LAST system-prompt section (`deployment:append`, order `1000`) — after the persona and every tool-guidance section. `{{variable}}` references are strict. Empty or omitted ⇒ no append section. This is the deployment-side form of an injected prompt: saved guidance lands at the end of the system prompt instead of in the user input. |
| `contextTokenBudget` | — | Token cap for the rendered runtime-context snapshot. When the rendered contexts exceed it, the highest-ordered contributions are kept and a `context:truncated` note is appended so the model knows the snapshot is partial. Omitted ⇒ no budgeting. Token estimates use the `estimateTokens` heuristic (`chars / 4`), so size the budget with headroom. Positive integer only; non-positive values fail at load. |
...
- `PromptSection` — `{ name, order, text, complete? }`. Sections are concatenated in ascending `order`. Order bands: `-100` is the harness identity, `0` the deployment persona, tool guidance uses `100–199`, `1000` the deployment append tail. One effective `complete` section suppresses all other sections after cooperative assembly.
- `PromptAssembly` — `{ sections: AssembledSection[], tools: ToolSchema[], variables: Record<string, string | undefined>, contextTokenBudget? }`. Section texts arrive resolved but not yet interpolated; `variables` holds every registered variable resolved against the context. Tool schemas are part of the assembly by design: "what the model is told it can do" is one coherent thing, even though adapters transmit schemas as a separate wire field. `contextTokenBudget` is the deployment's token cap for the rendered runtime-context snapshot, set from config and consumed by `renderContextSections`.
- `renderContextSections(assembly)` / `renderContextSnapshot(assembly)` — the runtime-context snapshot as named contributions / joined prose (the `Current runtime context…` block). When `contextTokenBudget` is set, the highest-ordered contributions that fit are kept and a `context:truncated` section names the omission count, so the model and any invariant companion see the snapshot is partial.
...
- Section providers: tool packages own their cross-call guidance (`tool:bash`, `tool:read`, …); this plugin owns `harness:identity`, `deployment:persona`, and the configured `deployment:append` tail.
